...
/**
 * DESCRIPTION :
 * Coquille commune des espaces membre et enfant.
 * - Desktop (md+) : sidebar fixe à gauche, comme avant.
 * - Mobile : sidebar masquée, remplacée par un bouton burger qui ouvre un tiroir
 *   glissant depuis la gauche (avec fond semi-transparent). Le tiroir se ferme
 *   automatiquement au changement de page.
 */
type Props = {
    /** Contenu de la sidebar (ex : <MemberSideMenu /> ou <ChildSideMenu />). */
    sidebar: ReactNode;
    /** Libellé affiché à côté du burger sur mobile. */
    titre: string;
    /** true si l'espace est sous le header global (h-16) → décale d'autant. */
    sousHeader?: boolean;
    children: ReactNode;
};
...
export default function DashboardShell({ sidebar, titre, sousHeader = false, children }: Props) {
    ...
    const top = sousHeader ? 'top-16' : 'top-0';
