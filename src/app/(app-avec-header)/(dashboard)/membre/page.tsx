...
const STATS: { Icon: LucideIcon; valeur: string; label: string; sub: string; bg: string; couleur: string }[] = [
    ...
    { Icon: TrendingUp, valeur: "37%", label: "Progression moyenne", sub: "tous enfants confondus", bg: "bg-[#f0fff4]", couleur: "text-[#00b894]" },
    { Icon: ClipboardList, valeur: "9", label: "Activités réalisées", sub: "depuis le début", bg: "bg-[#e8f4ff]", couleur: "text-[#2196F3]" },
    { Icon: Trophy, valeur: "6", label: "Badges obtenus", sub: "total famille", bg: "bg-[#fff8e8]", couleur: "text-[#d68910]" },
];

// Détail par enfant : "difficulte" = null si tout va bien, sinon liste de modules.
// Avatars en initiales (même style que la page « Mes enfants »).
const ENFANTS: {
    ...
    difficulte: string[] | null; activites: number; badges: number;
}[] = [
    {
        ...
        difficulte: null, activites: 6, badges: 4,
    },
    {
        ...
        difficulte: ["Lecture & compréhension"], activites: 3, badges: 2,
    },
];
...
// Conseils & astuces.
const CONSEILS: { Icon: LucideIcon; bg: string; couleur: string; titre: string; desc: string }[] = [
    { Icon: BookOpen, bg: "bg-[#f0fff4]", couleur: "text-[#00b894]", titre: "Encourager la lecture quotidienne", desc: "10 minutes de lecture par jour développent grandement le vocabulaire." },
    { Icon: Moon, bg: "bg-[#fff8e8]", couleur: "text-[#d68910]", titre: "Routine du soir sereine", desc: "Limiter les écrans 1h avant le coucher pour mieux dormir." },
    { Icon: Brain, bg: "bg-[#fff0fa]", couleur: "text-[#d6489f]", titre: "Concentration & écrans", desc: "Aider votre enfant à se concentrer en alternant écran et papier." },
];

// Style réutilisé pour les petits boutons de carte enfant.
const BTN = "inline-flex items-center gap-1.5 rounded-lg border border-[#d4cef0] bg-white px-3 py-1.5 text-[11px] font-semibold text-[#6d5ba8] transition-colors hover:bg-[#f4f1fb]";
    ...
    return (
        <div className="space-y-7">
            ...
            <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {STATS.map(({ Icon, valeur, label, sub, bg, couleur }) => (
                    <div key={label} className="flex items-center gap-3.5 rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(109,91,168,0.05)]">
                        <div className={`flex h-[46px] w-[46px] flex-shrink-0 items-center justify-center rounded-xl ${bg} ${couleur}`}>
            ...
            <section id="progression-detail" className="space-y-3.5 scroll-mt-20">
                <h3 className="flex items-center gap-2 text-lg font-bold text-[#2c2c3a]">
                    <BarChart3 className="h-5 w-5 text-[#6d5ba8]" aria-hidden /> Progression détaillée
                </h3>
                    ...
                    {ENFANTS.map((c) => (
                        <div key={c.username} className="rounded-[18px] bg-white p-6 shadow-[0_4px_16px_rgba(109,91,168,0.06)]">
                            ...
                            <div className="mb-4 flex items-center gap-3.5 border-b border-[#f0ecf8] pb-4">
                                <div className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br ${c.couleur} text-base font-bold text-white`}>
                                    {c.initiales}
                                </div>
                            ...
                            <div className="mb-3.5 grid grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
                                <div className="rounded-xl bg-[#faf8ff] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#9b8cff]">
                                        <TrendingUp className="h-3.5 w-3.5" aria-hidden /> Progression globale
                                    </div>
                                ...
                                <div className="rounded-xl bg-[#faf8ff] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#9b8cff]">
                                        <Star className="h-3.5 w-3.5" aria-hidden /> Module préféré
                                    </div>
                                    <div className="flex items-center gap-2 text-[15px] font-bold text-slate-800">
                                        <c.ModuleIcon className="h-5 w-5 text-[#6d5ba8]" aria-hidden />
                                ...
                                <div className="rounded-xl bg-[#faf8ff] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#9b8cff]">
                                        <Clock className="h-3.5 w-3.5" aria-hidden /> Temps passé
                                    </div>
                                ...
                                <div className="rounded-xl bg-[#faf8ff] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#9b8cff]">
                                        <Calendar className="h-3.5 w-3.5" aria-hidden /> Dernière activité
                                    </div>
                                    <div className="text-[15px] font-bold text-slate-800">{c.derniere}</div>
                                </div>

                                <div className="rounded-xl bg-[#fff8e8] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#d68910]">
                                        <Flame className="h-3.5 w-3.5" aria-hidden /> Série actuelle
                                    </div>
                                ...
                                <div className="rounded-xl bg-[#f0fff4] p-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-[#00b894]">
                                        <Target className="h-3.5 w-3.5" aria-hidden /> Quizz réussis
                                    </div>
                            ...
                            {c.difficulte ? (
                                <div className="rounded-xl border-l-4 border-[#d63031] bg-[#fff3f3] px-4 py-3.5">
                                    <div className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#d63031]">
                                        <AlertTriangle className="h-4 w-4" aria-hidden /> Difficultés détectées
                                    </div>
                                    ...
                                    <div className="mt-2 flex items-center gap-1.5 text-[11.5px] text-slate-500">
                                        <Lightbulb className="h-3.5 w-3.5" aria-hidden /> Encouragez {c.prenom} à refaire les leçons et exercices de ces modules.
                                    </div>
                                </div>
                            ) : (
                                <div className="rounded-xl border-l-4 border-[#00b894] bg-[#f0fff4] px-4 py-3.5">
                                    <div className="mb-1 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#00b894]">
                                        <CheckCircle2 className="h-4 w-4" aria-hidden /> Aucune difficulté détectée
                                    </div>
            ...
            <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                <div id="enfants" className="rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(109,91,168,0.05)] scroll-mt-20">
                    <div className="mb-3.5 flex items-center justify-between">
                        ...
                        <Link href="/membre/enfants/ajouter" className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#9b8cff] hover:text-[#6d5ba8]">
                    ...
                    {ENFANTS.map((c) => (
                        <div key={c.username} className="mb-3 rounded-xl bg-[#faf8ff] p-3.5">
                            ...
                            <div className="mt-2.5 flex justify-end gap-1.5">
                                <Link href="/membre/enfants" className={BTN}><BarChart3 className="h-3.5 w-3.5" aria-hidden /> Profil</Link>
                                <Link href="/membre/enfants" className={BTN}><Pencil className="h-3.5 w-3.5" aria-hidden /> Éditer</Link>
                                <button className="inline-flex items-center rounded-lg border border-[#fab1a0] bg-white px-3 py-1.5 text-[#d63031] transition-colors hover:bg-[#fff3f3]" aria-label="Supprimer">
                    ...
                    <div>
                        {ACTIVITES.map((a, i) => (
                            <div key={i} className="flex items-center gap-3 border-b border-[#f0ecf8] py-2.5 last:border-0">
                                <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-[#f4f1fb] text-[#6d5ba8]">
                ...
                <div id="conseils" className="rounded-2xl bg-white p-5 shadow-[0_4px_16px_rgba(109,91,168,0.05)] scroll-mt-20">
                    ...
                    <div className="grid gap-2">
                        {CONSEILS.map(({ Icon, bg, couleur, titre, desc }) => (
                            <div key={titre} className={`rounded-xl p-3.5 ${bg}`}>
                                <div className={`mb-2 flex h-8 w-8 items-center justify-center rounded-full bg-white ${couleur}`}>
