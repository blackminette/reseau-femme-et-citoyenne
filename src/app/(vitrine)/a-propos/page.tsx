...
const CHIFFRES = [
    { valeur: "500+", label: "Enfants accompagnés", couleur: "text-indigo-600" },
    { valeur: "15+", label: "Types d'ateliers", couleur: "text-emerald-600" },
    { valeur: "6", label: "Années d'expérience", couleur: "text-orange-500" },
    { valeur: "4", label: "Animateurs diplômés", couleur: "text-purple-600" },
];
...
const EQUIPE = [
    { initiales: "MD", nom: "Marie Dubois", role: "Responsable pédagogique", bio: "Directrice des ateliers avec 8 ans d'expérience en animation pour enfants. Passionnée par la créativité et l'épanouissement.", couleur: "from-indigo-400 to-purple-500" },
    { initiales: "TL", nom: "Thomas Lefevre", role: "Animateur arts & bricolage", bio: "Artiste plasticien, il encadre les ateliers de peinture, sculpture et création. Grand enfant dans l'âme !", couleur: "from-orange-400 to-pink-500" },
    { initiales: "SM", nom: "Sophie Martin", role: "Animatrice cuisine & nature", bio: "Chef pâtissière bienveillante, elle transmet le goût des bons produits et des moments de partage et de découverte.", couleur: "from-emerald-400 to-teal-500" },
    { initiales: "LB", nom: "Lucas Bernard", role: "Animateur expression & jeux", bio: "Formé au théâtre et à l'animation, il anime les ateliers d'éveil pour que chaque enfant puisse s'exprimer librement.", couleur: "from-sky-400 to-indigo-500" },
];
        ...
        <section id={id} className={`py-20 ${fond ? "bg-white" : ""}`}>
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <header className="text-center space-y-3 mb-14">
                    <h2 className="text-3xl font-extrabold text-slate-900">{titre}</h2>
    ...
    return (
        <div className="bg-slate-50">
            ...
            <section className="bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-700 text-white">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 grid md:grid-cols-2 gap-10 items-center">
                    <div className="space-y-6">
                        <span className="text-xs font-semibold tracking-widest uppercase text-indigo-200">Association</span>
                        <h1 className="text-5xl font-extrabold leading-tight">Notre association</h1>
                        <p className="text-lg text-indigo-100 max-w-md">Une équipe engagée pour l'épanouissement des enfants.</p>
                        <a href="#histoire" className={`${BTN} bg-white text-indigo-700 hover:bg-indigo-50`}>
                        ...
                        {ATOUTS.map(({ Icon, titre, texte }) => (
                            <div key={titre} className="flex gap-4 items-start bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-5">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-indigo-100">{texte}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 relative z-10">
                <div className="flex flex-wrap justify-center gap-3 bg-white rounded-full shadow-sm border border-slate-200 px-6 py-3 w-fit mx-auto">
                    {NAV.map(({ href, Icon, label }) => (
                        <a key={href} href={href} className="flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-indigo-600 px-3 py-1">
            ...
            <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20 grid md:grid-cols-2 gap-12 items-center">
                <div className="space-y-5">
                    <span className="text-xs font-semibold tracking-widest uppercase text-indigo-500">Qui sommes-nous ?</span>
                    <h2 className="text-3xl font-extrabold text-slate-900">Une association au service des enfants</h2>
                    ...
                    <Link href="#histoire" className={`${BTN} bg-indigo-600 hover:bg-indigo-700 text-white`}>
            ...
            <Section id="histoire" titre="Notre histoire" sous="De l'idée à l'association — six années d'aventure créative et humaine." fond>
                <div className="grid md:grid-cols-2 gap-x-12 gap-y-10">
                    {HISTOIRE.map(({ annee, Icon, titre, texte }) => (
                        <article key={annee} className="flex gap-4 border-l-2 border-indigo-100 pl-6 relative">
                            <span className="absolute -left-[9px] top-1 w-4 h-4 rounded-full bg-indigo-600 border-2 border-white" />
                            <div className="space-y-1">
                                <span className="text-xs font-semibold text-indigo-500">{annee}</span>
                                <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                    <Icon className="w-4 h-4 text-indigo-400 shrink-0" aria-hidden /> {titre}
                                </h3>
                    ...
                    {PARTENAIRES.map(({ Icon, nom, type, texte }) => (
                        <article key={nom} className={`${CARD} p-6 text-center hover:shadow-md transition-shadow`}>
                            <div className="w-14 h-14 mx-auto rounded-full bg-indigo-50 flex items-center justify-center">
                                <Icon className="w-7 h-7 text-indigo-500" aria-hidden />
                            </div>
                            <h3 className="mt-4 font-bold text-slate-900">{nom}</h3>
                            <p className="text-xs font-semibold tracking-wide uppercase text-indigo-500 mt-1">{type}</p>
                            <p className="text-sm text-slate-600 mt-3 leading-relaxed">{texte}</p>
                        </article>
                    ))}
                </div>

                <div className="mt-12 bg-gradient-to-br from-indigo-600 to-purple-600 rounded-2xl text-center text-white px-6 py-12">
                    <h3 className="text-2xl font-extrabold">Devenez partenaire</h3>
                    <p className="text-indigo-100 max-w-lg mx-auto mt-3">Vous souhaitez soutenir nos actions et contribuer à l'épanouissement des enfants de notre région ? Rejoignez notre réseau de partenaires !</p>
                    <Link href="/contact" className={`${BTN} mt-6 bg-white text-indigo-700 hover:bg-indigo-50`}>
            ...
            <Section id="equipe" titre="Notre équipe" sous="Des personnes bienveillantes, créatives et engagées auprès de vos enfants." fond>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {EQUIPE.map(({ initiales, nom, role, bio, couleur }) => (
                        <article key={nom} className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
                            ...
                            <div className="p-5">
                                ...
                                <p className="text-sm font-medium text-indigo-500">{role}</p>
                    ...
                    {AVIS.map(({ note, texte, auteur, role }, i) => (
                        <article key={i} className={`${CARD} p-6 flex flex-col`}>
                            ...
                            <footer className="mt-4 pt-3 border-t border-slate-100">
                                <p className="text-sm font-semibold text-slate-900">{auteur}</p>
