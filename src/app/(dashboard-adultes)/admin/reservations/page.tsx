// * src/app/(dashboard-adultes)/(dashboard)/admin/reservations/page.tsx
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-800 p-8">
            ...
            <div className="flex flex-col gap-4 border-b border-slate-200 pb-5">
                <Link href="/admin" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-indigo-600 transition-colors w-fit">
                    <ArrowLeft className="h-4 w-4" />
                    Retour à la console
                </Link>
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Gestion des Réservations</h1>
                    <p className="text-sm text-slate-500 mt-1">Validez ou annulez les inscriptions des membres et de leurs enfants aux ateliers.</p>
                </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl">
                <div className="bg-white border border-slate-200 p-6 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                        {isLoading ? <div className="h-8 w-12 bg-slate-100 animate-pulse rounded" /> : <span className="text-2xl font-bold text-slate-900">{stats.aujourdhui}</span>}
                        <p className="text-sm font-medium text-slate-500">Demandes aujourd'hui</p>
                    </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-6 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                        {isLoading ? <div className="h-8 w-12 bg-slate-100 animate-pulse rounded" /> : <span className="text-2xl font-bold text-slate-900">{stats.enAttente}</span>}
                        <p className="text-sm font-medium text-slate-500">En attente (PENDING)</p>
                    </div>
                </div>
                ...
                <div className="bg-white border border-slate-200 p-6 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                        {isLoading ? <div className="h-8 w-12 bg-slate-100 animate-pulse rounded" /> : <span className="text-2xl font-bold text-slate-900">{stats.total}</span>}
                        <p className="text-sm font-medium text-slate-500">Total historique</p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-8 max-w-5xl flex flex-col sm:flex-row gap-4 items-center justify-between">
                <div className="relative w-full sm:w-96">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-slate-200 pl-10 pr-4 py-2.5 rounded-xl text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-slate-800 placeholder-slate-400"
                    />
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                    ...
                    <select
                        ...
                        className="bg-white border border-slate-200 px-3 py-2.5 rounded-xl text-sm focus:outline-none focus:border-indigo-500 text-slate-700 font-medium"
                    >
            ...
            {/* TABLEAU */}
            <div className="mt-4 max-w-5xl bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
                            ...
                            {isLoading ? (
                                Array.from({ length: 3 }).map((_, idx) => (
                                    <tr key={idx} className="animate-pulse">
                                        <td className="p-4 pl-6"><div className="h-4 bg-slate-100 rounded w-32 mb-2" /><div className="h-3 bg-slate-100 rounded w-24" /></td>
                                        ...
                                        <td className="p-4 pr-6 text-right"><div className="h-8 bg-slate-100 rounded-lg w-20 ml-auto" /></td>
                                    </tr>
                                ))
                            ) : reservationsFiltrees.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-slate-400 font-medium">Aucune réservation trouvée.</td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={res.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="p-4 pl-6">
                                            <div className="flex flex-col">
                                                ...
                                                {res.enfantNom && (
                                                    <span className="text-xs text-indigo-600 bg-indigo-50/60 font-medium px-1.5 py-0.5 rounded mt-1 w-fit">
                                        ...
                                        <td className="p-4 text-slate-600 font-medium">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xs text-slate-400 font-normal">à {res.heureAtelier}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="p-4 pr-6 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                                                {res.statut === 'PENDING' && (
                                                    <>
                                                        <button
                                                            ...
                                                            className="px-2.5 py-1.5 text-xs font-bold bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white rounded-lg transition-colors"
                                                        >
                                                            Accepter
                                                        </button>
                                                        <button
                                                ...
                                                {res.statut === 'CONFIRMED' && (
                                                    <button
                                                        onClick={() => modifierStatut(res.id, 'CANCELLED')}
                                                        className="px-2.5 py-1.5 text-xs font-medium border border-slate-200 hover:border-rose-200 text-slate-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all"
                                                    >
                                                        Annuler
                                                    </button>
                                                )}
